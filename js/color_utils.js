/**
 * Shared Color Utilities for ComfyAngel
 *
 * Common functions for color manipulation and color picker dialog.
 */
...
export function getConnectedImageUrl(node, visited = new Set()) {
...
export function getImageFromNode(node) {
...
/**
 * Show color picker dialog with optional image eyedropper
 *
 * @param {string} currentColor - Initial color (hex format)
 * @param {function} onApply - Callback when color is applied: (hex) => void
 * @param {string|null} imageUrl - Optional image URL for eyedropper
 */
export function showColorPickerDialog(currentColor, onApply, imageUrl = null) {
